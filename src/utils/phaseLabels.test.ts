...
import { formatPhaseShortLabel, pluralizeHolds, formatPhaseDisplayName } from './phaseLabels';
...
describe('phaseLabels', () => {
  describe('formatPhaseShortLabel', () => {
    ...
    it('returns first word for non-hold', () => {
      expect(formatPhaseShortLabel({ type: 'prepare', label: 'Controlled Inhalation' })).toBe(
        'Controlled'
      );
    });
  });
