...
export const Loader = memo(function Loader({ label, size = 'md', className }: LoaderProps) {
  ...
  const spinner = (
    ...
    >
      <span
        className={clsx('animate-spin text-primary/70 flex items-center justify-center', innerSize)}
      >
  ...
  return (
    <span className={clsx('inline-flex items-center justify-center', className)}>{spinner}</span>
  );
});
