...
describe('timerEngine', () => {
  ...
  it('relaxationSecondsOverride shortens relaxation', () => {
    ...
    const cb = vi.fn();
    engine.on('phase_start', cb);
    engine.start([{ type: 'hold', duration: 5 }], { relaxationSecondsOverride: 2 });
    vi.advanceTimersByTime(100);
    expect(cb).not.toHaveBeenCalled();
  ...
  it('getState returns correct phase and remainingMs', () => {
    const engine = createTimerEngine();
    engine.start([{ type: 'hold', duration: 10 }], { relaxationSecondsOverride: 1 });
    vi.advanceTimersByTime(100);
