...
function loadCue(cue: CueName): Promise<HTMLAudioElement> {
  return new Promise((resolve, reject) => {
    ...
    audio.onerror = () =>
      reject(
        new Error(
          `Audio file ${CUE_FILES[cue]} failed to load. Ensure hold.m4a, prepare.m4a, 30s.m4a, breathe.m4a exist in public/audio/`
        )
      );
  });
}
  ...
  async function preload(): Promise<void> {
    ...
    if (firstRejection) {
      throw firstRejection.reason instanceof Error
        ...
        : new Error(
            'Audio file failed to load. Ensure hold.m4a, prepare.m4a, 30s.m4a, breathe.m4a exist in public/audio/'
          );
    }
