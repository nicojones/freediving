...
key-files:
created: - server/index.js - server/db.js - server/schema.sql - server/auth.js - server/routes/auth.js - server/routes/progress.js - src/services/authService.ts - src/services/progressService.ts - src/pages/LoginPage.tsx
modified: - package.json - vite.config.ts - src/App.tsx - .gitignore
decisions:

- JWT in httpOnly cookie for session; 7-day expiry
- Pre-defined users (nico, athena) seeded with bcrypt; env USER*PASSWORD*\* for dev
